...
| `pull-quejas.yml`                 | daily 04:00 UTC (before the scrape) — pulls the bot's `/export/quejas.json` from Fly.io into `public/data/`. Live since 2026-08-02. Needs `vars.BOT_EXPORT_URL`: without it the run fails (`exit 1`) and its step summary says how to set it. Pulls only `quejas.json`: the anonymized photos under `public/data/quejas-photos/` are written and pruned by `npm run process-photos` in `bot/`, which no workflow, cron or Fly process runs |
