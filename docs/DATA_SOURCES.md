...
- **Corrections** — `officials.json` is scraped ⊕ corrected. The council's page lags what its own Pleno already decided (a resignation of May 2025 was still unreflected in September 2026), so `officials-corrections.json` — curated, each entry citing the acta verbatim and signed — is applied by `scripts/scrape-officials.ts` after parsing (`composeOfficialsSnapshot`). The published file carries `officials[]` (sitting; `count` and `composition` derive only from these), `formerOfficials[]` (with `until`, `reason`, `replacedBy`, `source`) and a `corrections` block. An `alta` row carries `correccion: {tipo, since, source}` and `photoNote` when the source publishes no portrait; `email: null` means «sin correo publicado». When the page cannot be scraped (403 since 2026-09-02) `npm run roster-correction -- --apply` recomposes the published file from its own rows. `check:officials-corrections` asks two questions with four outcomes each: was it applied (`aplicada` / `no-aplicada`, blocks) and is it still needed against the live page (`vigente` / `absorbida` → retire it / `contradicha` / `NO COMPROBADO` when the page does not answer).
...
- **Source** — Daily job. HEAD-checks every unique `articleUrl` in `press-claims-suggestions.json`, calls Wayback's Availability API to find an existing snapshot, optionally Save Page Now for dead/forced URLs — only when the lookup ANSWERED that none exists, never when it was refused. **A Wayback copy does not vanish, and the snapshot used to say it did**: measured on 2026-09-20 over the last fourteen runs, most of the URLs that showed a copy in some run «lost» it in a later one and got it back afterwards — on `/promesas`, a citation's archived-copy link blinking from one day to the next, and a `stats.archived` that wandered over the same URL set. The API fails to report a copy in two ways, both seen that day: it refuses the lookup (429), or it answers a clean 200 with `archived_snapshots: {}` for a URL that has a capture. So the copy an earlier run found is kept until a lookup finds another (`archiveCarried: true`), no save is spent on a page that has one, each row says what Wayback answered today (`archiveLookup: found\|saved\|none\|failed` — `failed` is «could not look», never «no copy»), and `stats` counts `archivedCarried`, `lookupNone` and `lookupFailed` beside `archived`: a low `archived` with a high `lookupFailed` is «could not look today», not «few copies». The limit: a capture Wayback has since excluded keeps being linked, and nothing checks those links yet. Writes `{articleUrl, status: alive\|dead\|error, archivedUrl, archivedAt, archiveLookup, archiveCarried?, checkedAt, …}`. Chained nightly after `auto-curate-press`.
...
- **Withdrawal** — `npm run retract-vote` is the only way out. Three scopes, because the
  ...
  paraphrased a Whisper line about a grant application deadline. `/plenos` names the scope
  in its retraction sentence and `/datos` counts it, but **the row's own `dueByRetracted`
  stamp is rendered nowhere**: on `/departamentos/:slug` the vote simply shows no deadline,
  where a withdrawn breakdown shows `VoteBreakdownRetracted` in the tally's place. Retractions are tombstoned into `retractions[]` with the
