...
- **ClaimReview JSON-LD publishing.** `src/components/ClaimReviewJsonLd.jsx` emits schema.org/ClaimReview structured data per finding on `/laboratorio` (press) AND `/hallazgos` (pleno) — two builders (`_buildPayload` + `_buildPlenoPayload`) discriminated by shape. We now *publish* in the same standard the Google Fact Check Tools API indexes (and that we *read* from third-party fact-checkers). Severity maps to a 1-5 reviewRating with bilingual alternateName.
