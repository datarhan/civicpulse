...
- **Verifier hardening (2026-04-30).** LLM second pass over `sin-datos` claims now defaults to hybrid lexical-∪-semantic shortlist; prompt v2 demands structured cites of the form `<dataset>[i].<field>=<value>` whose value must literally appear in the candidate snippet — citations failing either check are rejected as hallucinations. Telemetry (`out-of-range`, `missing-cite`, `cite-not-in-snippet`) reports per run. Embeddings backend is now pluggable: `EMBED_BACKEND=openai|gemini` (Gemini text-embedding-004, free tier, no PAYG required) so the semantic shortlist no longer depends on OpenAI quota. `/metodologia#verificacion-declaraciones` updated with the new contract.
...
**Newer still (2026-05) — Press-lab v2**
- **Verifier recall fix.** Lowered `cita_obra` entity-overlap from 0.65 → 0.50; extended `tenderTitle()` to concat `title · contractor · assignee · categoryTitle`; widened bilingual STOPWORDS with debate verbs (presenta/solicita/demanda/insta/anuncia/comparece/sol·licita/demana/…); extended entity-only match to `acusacion_publica` with factual/contra-datos subtypes. Acceptance test in `tests/claim-verifier-recall.test.ts` locks the path: a synthetic press claim matching a real TED notice now verifies as `verificado` with similarity=1.0.
- **Four new data sources.**
  - **EU TED tenders** (`tenders-ted.ts` + `scrape-tenders-ted.ts`) — v3 API at `api.ted.europa.eu/v3/notices/search`, free no-auth, expert-query `buyer-name~"Riba-roja"`. Projects each row onto the same `TenderRow` shape via `asTenderRow()` so the existing amount-based verifier consumes them with `[...local.contracts, ...ted.items]`. 54 EU notices indexed.
  - **BOE adapter** (`boe.ts` + `scrape-boe.ts`) — walks the last 30 days of `boe.es/datosabiertos/api/boe/sumario/{YYYYMMDD}`, filters titulos to Riba-roja mentions. New `applyBoeCrossRef` step in the press-verifier upgrades sin-datos → verificado when an acto-administrativo trigger word (ordenanza/decreto/resolución/convenio/expropiación/sanción/plan general/subvención) overlaps a BOE titulo.
  - **Maldita + Newtral RSS supplement to factcheck.json** — `parseFactcheckRss` + `mergeFactCheckRows` extend `factcheck.ts`; `scrape-factcheck.ts` now fetches both publisher RSS feeds in parallel with the Google FCT API call. Graceful degrade: missing API key still runs RSS; broken RSS still keeps API rows.
  - **Catastro curator lookup** (`catastro.ts` + `lookup-catastro.ts`) — wraps OVC's `Consulta_DNPLOC` (by address) and `Consulta_DNPRC` (by RefCat). NOT wired into the auto-verifier — libel discipline keeps the human in the loop because the WCF stack is brittle.
- **Wayback archival + link-rot audit.** `wayback.ts` exposes `archiveOnWayback` (Save Page Now) + `findExistingSnapshot` (Availability API). `audit-press-links.ts` walks every press `articleUrl`, HEAD-checks, looks up an existing snapshot, optionally forces SPN for dead URLs. Chained nightly after `auto-curate-press`. `/laboratorio` surfaces a "🔗 Wayback ↗" link per card; "Ver original" flips red ⚠︎ when the audit marked the URL dead.
- **ClaimReview JSON-LD publishing.** `src/components/ClaimReviewJsonLd.jsx` emits schema.org/ClaimReview structured data per finding on `/laboratorio` (press) AND `/hallazgos` (pleno) — two builders (`_buildPayload` + `_buildPlenoPayload`) discriminated by shape. We now *publish* in the same standard the Google Fact Check Tools API indexes (and that we *read* from third-party fact-checkers). Severity maps to a 1-5 reviewRating with bilingual alternateName. **Superseded 2026-08-09:** the severity→rating projection was a category error — `severity` grades our finding, not the truth of the quoted sentence, and `informational` (the auto-curator default) shipped politicians' own words to Google's fact-check index as CivicPulse-rated 5/5 «Verificado». The component now emits only for a finding carrying a `contradiction[]` ref, rating it contradicted; see the header of `ClaimReviewJsonLd.jsx` and `/metodologia#…ClaimReview`.
- **IFCN-compliant correction trails.** `PressFinding.corrections[]` + `PlenoFinding.corrections[]` (symmetric schemas). Per-correction record `{field: title|summary|severity, original, corrected, reason ≥20 chars, editor, correctedAt: ISO}`. Two CLIs (`correct-press-finding`, `correct-pleno-finding`) append + re-validate the whole snapshot before writing. Both surfaces render a collapsible "Bitácora de correcciones" with strike-through diff.
- **Editorial methodology section.** `/metodologia#laboratorio-prensa` covers verdict scale, libel discipline, Wayback preservation, ClaimReview publishing, corrections policy, audited sources, and the explicit IFCN-five-pillar mapping. The `methodologyUrl` field that `validatePressFindingsSnapshot` was already pointing at now lands on a real section.

**Net assessment.** The core editorial loop (transcript → extraction → verification → curated finding → right-of-reply) is shipped end-to-end and exercised on real plenos. The two remaining gates before public-launch are (a) Fly.io migration of the bot (deferred deliberately) and (b) cross-bloc voiceprint enrollment (needs human-supplied URLs). The press-lab v2 surface is now signatory-ready under IFCN principles; submitting the application is an editorial decision, not engineering work.
