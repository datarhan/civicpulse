...
npm run scrape:obras                 # Obras de infraestructura (14 fichas: 7 FEDER 2019-20 + 7 Plan RENOVE 2023-24 · importes/empresa + geo vía zona afectada) → obras.json → /presupuesto + map
...
npm run scrape:empleo               # Open job vacancies · Agència de Col·locació (ADL) · portalemp CSRF handshake + per-offer ficha
                                    #   → writes public/data/empleo.json + empleo-rss.xml (RSS 2.0 feed, 60 newest)
...
npm run check:relations             # cross-snapshot FK audit (findings→claims, manifest→chunks,
                                    ...
                                    # dept slugs, entities→contracts; warn: stale overlay entries,
                                    # votes↔agendas, dedicaciones slugs, stale entity aliases) ·
                                    # strict exits 1 on error-level breakage · scrape-all runs it
                                    # with --soft (report-only)
npm run compute:entities            # canonical entity registry (companies from tenders assignees,
                                    # legal-form-family normalization SL≠SA, + people from officials)
                                    # → entities.json · runs in scrape:all · curated aliases applied
npm run entity-alias -- "<variant>" "<canonical>" [--note …] [--curator …]
                                    # curator merge of razón-social variants → entity-overrides.json
...
raw image _in memory_ (never to disk), runs a **vision model** (gemini-first via
...
- `src/components/LiveCity/StylizedMap.jsx` — the `/` landing map, decomposed into a thin orchestrator + `network/` (boundary, Railways, FullNetwork, NetworkLegend), `popups/` (StationSchedulePopup, GtfsSchedulePopup, PlacePopup, NeighborhoodPopup), `layers/` (MoneyLayer, NeighborhoodsLayer, CivicPoiLayer, FloodRiskLayer, QuejasLayer), and `controls/` (LayerControl, MoneyTimeSlider, PoiLegend, FloodLegend, QuejasLegend). A custom bottom-left `LayerControl` toggles data layers over the CartoDB Voyager base; each layer is conditionally mounted so a hidden layer's rAF/WMS never runs. **Layers:** civic services (OSM POI, **on by default** — the landing flagship); "Gasto municipal" precise obras-situadas pins (tender-geo `places[]`, one `€`-scaled `CircleMarker` per resolved street/POI/urbanización/barrio + a 2018→2025 time-slider + DANA + "Solo obras" (construction-only) toggles + click-to-drill `PlacePopup` → shared `ContractCard`, off/static); interactive neighborhoods (the 21 OSM barrios open an aggregated civic card — population + located spend + quejas, always on); citizen-quejas heat (one health-coloured `Circle` per barrio with ≥1 queja, radius ∝ √count, reuses the /quejas heatmap join, off); PATRICOVA flood-risk WMS (off). **No synthetic buildings, no fake scores** — every pin traces to a contract whose title named that place (see the place-resolver's honesty gates below); honest empty-states (a barrio with zero quejas never paints). Pure logic is unit-tested in `src/lib/{tender-geo,tender-points,neighborhood-aggregate,civic-poi,cpv,tenders}.js`.
...
scripts/compute-entities.ts           →  src/scraper/entities.ts          →  public/data/entities.json
...
public/data/entity-overrides.json    (schema: src/scraper/entities.ts validateEntityOverrides · curator razón-social alias merges via npm run entity-alias · applied by compute:entities)
...
| Obras municipales (fichas 2019–2024)                                                                                           | `obras.ts` → `obras.json`                                                                  | Portal de Transparencia · obras-de-infraestructuras-en-curso fichas (FEDER 2019–20) + urbanismo/vias_y_obras Plan RENOVE fichas (2023–24, zona afectada → gazetteer geo)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `/presupuesto` section + landing-map layer                                                                                                                                                                                                                                                                                       |
...
| Civic POIs (34 named: schools/health/parks/sport/culture/civic incl. townhall/cemetery)                                        | `civic-poi.ts` → `civic-poi.json`                                                          | **OSM Overpass API** — `amenity`/`leisure`/`healthcare`/`tourism` civic tags inside the `wikidata=Q23701` area. Requires a `name` on the noisy `leisure` bucket so the ~1.5k private backyard pools never reach the map; parser drops unnamed + dedups node/area                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Direction D StylizedMap "Servicios" layer (category-coloured markers + legend)                                                                                                                                                                                                                                                   |
| Flood-risk zones (no snapshot — live WMS)                                                                                      | _(none — WMS overlay)_                                                                     | **PATRICOVA** WMS · Generalitat Valenciana / ICV `carto.icv.gva.es/…/WMSServer` layer 59 "Riesgo de Inundación" (EPSG:3857)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Direction D StylizedMap "Riesgo inundación" layer (`WMSTileLayer`, off by default)                                                                                                                                                                                                                                               |
...
| Sindicatura de Comptes CV fiscalización reports (ex-post audit court)                                                          | `sindicatura.ts` (+ `sindicatura-findings.ts`) → `sindicatura.json`                        | **Sindicatura de Comptes CV** `/informes` search (`text=Riba-roja de Túria&type=full`, indexes PDF content). Parser classifies each result row: `dedicated` (TITLE names the town) vs `sectoral` (Riba-roja inside a local-entities sweep, `isLocalEntityReport` filter). The CLI also **pdf-parses** the dedicated "control interno" report into structured findings — the 27 numbered _salvedades_ grouped by area + the 5 recomendaciones (`parseAuditFindings`) — and pulls Riba-roja's **art. 218 rendition row** (En plazo / ACR _acuerdos contrarios a reparos_ / OFP / AI) from the annual control-interno EELL report (`parseControlInternoArt218`, the freshest signal). curl-reachable (unlike PLACSP).                                                                                                                                                                                                                                                                                 | `/quejas` `SindicaturaCard` — the ex-post audit pillar next to Síndic + CTBG; art. 218 status box (freshest), dedicated audits with a "27 deficiencias" expander, sectoral in a collapsible                                                                                                                                      |
...
- `useEntities` — canonical entity registry (companies + people); feeds the /presupuesto contractor leaderboard's razón-social merge (404 → raw-name behavior)
...
- `public/data/promise-suggestions.json` is **machine-written** by the
  ...
  _never_ substitute for the published status.
...
mutate _only_ `frozenUntil` + `generatedAt` and re-validate the whole
