 ...
 * `/hallazgos`, which transcript it actually comes from.
 *
 ...
 * snapshot derived from other snapshots, so the SPA reads ~17 KB instead of
 * fetching 300 KB of transcript per session to answer the same question.
 *
 ...
 *
 * The decision tree lives in `src/scraper/quote-provenance.ts` and is shared
 * with `check:finding-quotes`, which re-derives it and refuses to pass if the
 * committed snapshot disagrees. One matcher, one classifier, two callers.
 */
...
export function computeProvenance(now = new Date()): QuoteProvenanceSnapshot {
  ...
  return buildQuoteProvenance(snapshot.items, sessions, {
...
function main() {
  ...
  const sanity = provenanceSanityFailure(s)
  if (sanity) {
